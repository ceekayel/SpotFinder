...
{ 
	...
	add_image_size( 'directory-listing-image', 450, 300, true );
	add_image_size( 'event-listing-image', 450, 300, true );
	add_image_size( 'property_listings-thumb', 450, 300, true );
	update_option( 'thumbnail_size_w',450);
	update_option( 'thumbnail_size_h',300);
	set_post_thumbnail_size( 450, 300, $crop );
	...
	add_image_size( 'directory-single-image', 500, 320, true );
	add_image_size( 'directory-single-thumb', 77, 77, true );

	add_image_size( 'directory-neighbourhood-thumb', 77, 77, true );


	update_option('thumbnail_size_w',450);
	add_filter('thumbnail_size_h',300);

	add_image_size('tevolution_thumbnail',77,77,true);
	add_image_size('popular_post-thumbnail',77,77,true);
	
	add_image_size( 'event-listing-image', 450, 300, true );
	add_image_size( 'event-single-image', 500, 320, true );
	add_image_size( 'event-single-thumb', 77, 77, true );

	add_image_size( 'event-neighbourhood-thumb', 77, 77, true );
	add_image_size( 'directory-single-thumb', 77, 77, true );
	add_image_size( 'popular_post-thumbnail',77,77,true);
	add_image_size( 'event-single-thumb',77,77, true);
	add_image_size( 'tevolution_thumbnail',77,77,true);
...
function tmpl_header_city_image(){
	...
	<?php 
		elseif($current_cityinfo['header_image']):?>
		<style>body{ <?php if($current_cityinfo['color']):?> background-color:<?php echo $current_cityinfo['color'];?>; <?php endif;?> <?php if($current_cityinfo['images']):?> background-image:url('<?php echo $current_cityinfo['images'];?>'); <?php endif;?> }
		div#header, header#header{ background:none; !important }
		.directory-front-page .home_page_banner{ <?php if($current_cityinfo['header_color']):?> background-color:<?php echo $current_cityinfo['header_color'];?>; <?php endif;?> <?php if($current_cityinfo['header_image']):?>background-image:url('<?php echo $current_cityinfo['header_image'];?>'); <?php endif;?> <?php if($current_cityinfo['header_color']=='' && $current_cityinfo['header_image']=='' ): ?>background-image:url('<?php echo get_stylesheet_directory_uri()."/images/home-banner-bg.jpg";?>'); <?php endif; ?> } </style>
		<?php
		elseif($header_image && $current_cityinfo['header_image']==''): ?>
		<style>body{ <?php if($current_cityinfo['color']):?> background-color:<?php echo $current_cityinfo['color'];?>; <?php endif;?> <?php if($current_cityinfo['images']):?> background-image:url('<?php echo $current_cityinfo['images'];?>'); <?php endif;?> }
		div#header, header#header{ background:none; !important}
...
function spotfinder_remove_sidebar(){
	...
	if($args):
		...
		{
			if(in_array($_args['post_type'],$exclude_array) || file_exists(get_stylesheet_directory()."/taxonomy-".$_args["labels"]["singular_name"].".php")){
...
function tmpl_get_related_listings(){
	...
	$related_posts = tmpl_get_related_posts_query();
		...
		echo "<h2>"; _e('Related Listings',TDOMAIN); echo "</h2>";
...
function tpml_spot_finder_classes($classes) {
	...
	if((is_tax() || is_tag || is_search()) && !is_single() && !is_category() && !is_page() && !is_author() && !in_array($post_type,$exclude_array1) && in_array($post_type,$exclude_array) && !isset($_REQUEST['page']) && !isset($_REQUEST['pmethod']) && $wp_query->is_posts_page !=1){
