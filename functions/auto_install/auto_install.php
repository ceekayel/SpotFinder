...
function activate_eco_plugin(){
	...
	if(!is_plugin_active('Tevolution/templatic.php') && is_admin() ){
		$current_system = "<a id='templatic_plugin' href=".$url." style='color:#21759B'>".__('Tevolution',TDOMAIN)."</a>";
	}	
	if(!is_plugin_active('Tevolution-Directory/directory.php') && is_admin() ){
		if($current_system != '')
			$current_system .= __(' and ', TDOMAIN);
		$current_system .= '<a id="booking_plugin" href="'.$url.'" style="color:#21759B">'.__('Tevolution - Directory',TDOMAIN).'</a>';
	}
...
<div class="error" style="padding:10px 0 10px 10px;font-weight:bold;"> <span>
  <?php echo sprintf(__('Thanks for choosing templatic themes, the base system of templatic is not installed at your side, Please download and activate %s addons to get started with %s website.',TDOMAIN),$current_system,'<span style="color:#000">'. @wp_get_theme().'</span>');?>
  </span> </div>
<?php 	
	endif;
}
...
function supreme_autoinstall()
{
	...
	if((strstr($_SERVER['REQUEST_URI'],'themes.php') && !isset($_REQUEST['page'])) && @$_REQUEST['template']==''){
		...
		$dummydata_title .= '<h3 class="twp-act-msg">'.__('Thank you. &quot;SpotFinder&quot; is activated.',ADMINDOMAIN).' <a href="'.site_url().'">Visit Site</a></h3>';
		
		/* theme message */	
		$dummy_theme_message .='<div class="tmpl-wp-desc">This theme allows you to &quot;create, manage and monetize a directory website&quot;. Make sure to refer <a href="http://templatic.com/docs/directory-theme-guide/">Installation Guide</a> of this theme to understand how to manually setup the theme and how different settings of this theme works. To get started, we&acute;ve outlined a few suggested steps to help you with this site. If you would like some help, get in touch with us at our <a href="http://templatic.com/forums/viewforum.php?f=119">community forum</a> or write to us at <a href="http://templatic.com/docs/submit-a-ticket/">helpdesk</a>.</div>';
		
		/* guilde and support link */	
		
		$dummy_nstallation_link  = '<div class="tmpl-ai-btm-links clearfix"><p>The team at <a href="http://templatic.com">templatic.com</a></p><ul><li>Need Help?</li><li><a href="http://templatic.com/docs/directory-theme-guide/">Installation guide</a></li><li><a href="http://templatic.com/forums/viewforum.php?f=140">Support Forum</a></li><li><a href="http://templatic.com/docs/submit-a-ticket/">HelpDesk</a></li></ul></div>';;
		if($post_counts>0){
			$theme_name = get_option('stylesheet');
			
			$dummy_data_msg='';
			$dummy_data_msg = $dummydata_title;
			$dummy_data_msg .= '<div class="tmpl-auto-install-yb" ><h4>'.__("Wish to remove sample data inserted by this theme?",ADMINDOMAIN).' </h4> <span><a class="button button-primary delete-data-button" href="'.site_url().'/wp-admin/themes.php?dummy=del">'.__('Delete sample data',ADMINDOMAIN).'</a></span> <p>Please note that deleting sample data will also remove any content you may have added or edited on any sample content. Deleting the sample data will also remove all sample widgets which were inserted.</p>';
			$dummy_data_msg .='</div>';
			...
			$dummy_data_msg .='<div class="ref-tev-msg">'.__('Please refer to &quot;tevolution&quot; and other sections on left side menu for rest of the advanced options.',ADMINDOMAIN).'</div>';
			...
			$dummy_data_msg='';
			$dummy_data_msg = $dummydata_title;
			$dummy_data_msg .= '<div class="tmpl-auto-install-yb" ><h4>'.__("Your site not looking like our online demo?",ADMINDOMAIN).' </h4> <span><a class="button button-primary" href="'.site_url().'/wp-admin/themes.php?dummy_insert=1">'.__('Install sample data',ADMINDOMAIN).'</a></span> <p>So that you don&prime;t start on a blank site, the sample data will help you get started with the theme. The content includes some default settings, widgets in their locations, pages, posts and a few dummy listings.</p>';
			$dummy_data_msg .='</div>';
			$dummy_data_msg .= apply_filters('tmpl_after_install_delete_button',$dummy_data_msg);
			$dummy_data_msg .= $dummy_theme_message;
		...
		if(isset($_REQUEST['dummy_insert']) && $_REQUEST['dummy_insert']){
			...
			if(!$nav_menus){
				...
				if(!in_array('secondary',$navmenu)){
					...
					wp_spotfinder_insert_name_menu_auto_install($secondary_post_info,'secondary');
...
function supreme_delete_dummy_data()
{
	...
	delete_option('map_customizer');
	delete_option('google_map_customizer');
}
...
function wp_spotfinder_insert_name_menu_auto_install($post_info,$menu_type){	
	...
	$nav_menu_locations = get_option('theme_mods_SpotFinder');
	$nav_menu_locations['nav_menu_locations'][$menu_type]=$term_id;	
	update_option('theme_mods_SpotFinder',$nav_menu_locations);
}

/*Display dummy data insert and delete notification msg on tevolution overview box  */
add_filter('tevoluton_overviewbox_datacontent','spotfinder_tevoluton_overviewbox_datacontent');
function spotfinder_tevoluton_overviewbox_datacontent($dummy_data_msg){
	global $wpdb;	
