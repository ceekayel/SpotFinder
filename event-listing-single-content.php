...
jQuery(function() {
 ...
 <?php
	...
	<ul class="tabs" data-tab role="tablist">
		...
		<li class='tab-title active'><a href="#event_image_gallery"><?php _e('Photos',EDOMAIN);?></a></li>
		<?php endif;

		if($video!="" && $tmpl_flds_varname['video'] || ($is_edit==1 &&  $tmpl_flds_varname['video'])):?>
          	<li class='tab-title'><a href="#event_video"><?php echo $tmpl_flds_varname['video']['label'];?></a></li>
        <?php endif;
	...
	<?php endif;
		...
		if(!empty($post_img) && $is_edit==''):?>
          ...
          <section role="tabpanel" aria-hidden="false" class="content active" id="event_image_gallery">
          	<div  id="slider" class="event_image flexslider">
                    ...
                    ?>
							<li>
                              	<a itemprop="image" href="<?php echo $value['file'];?>" title="<?php echo $img_title; ?>"  class="listing_img" >		
		...
		<section role="tabpanel" aria-hidden="false" class="entry-header-image content active" id="event_image_gallery">
        <!--editing post images -->
	...
	<h2><?php _e('Organizers',EDOMAIN);?></h2>
     ...
     <div class="event-organizer-left" >
        ...
        <div id="fronted_upload_organizer_logo" class="frontend_uploader button" data-src="<?php echo $org_logo?>">	                 	
            <span><?php echo __( 'Upload ', ADMINDOMAIN ).$tmpl_flds_varname['organizer_logo']['label']; ?></span>						
        </div>
     </div>
     <?php endif;?>
     ...
     <?php if( @$org_address!='' && @$tmpl_flds_varname['organizer_address'] || ($is_edit==1 &&  @$tmpl_flds_varname['organizer_address'])):?>
     ...
     <?php if($org_contact!='' && $tmpl_flds_varname['organizer_contact'] || ($is_edit==1 && $tmpl_flds_varname['organizer_contact'])):?>
     ...
     <?php if($org_mobile!='' && $tmpl_flds_varname['organizer_mobile'] || ($is_edit==1 && $tmpl_flds_varname['organizer_mobile'])):?>
     ...
     <?php if($org_email!='' && $tmpl_flds_varname['organizer_email'] || ($is_edit==1 && $tmpl_flds_varname['organizer_email'])):?>
...
<?php endif;?>
<?php if(isset($reg_desc) && $reg_desc!='' && @$tmpl_flds_varname['reg_desc'] || ($is_edit==1  && @$tmpl_flds_varname['reg_desc'])):?>
		<?php if($is_edit==1  && @$tmpl_flds_varname['reg_desc']):?>
...
if(!empty($htmlvar_name)){
	...
	foreach($htmlvar_name as $key=>$value){
		...
		foreach($value as $k=>$val){			
			$heading_key = ($key=='basic_inf')? __('Event Information',TDOMAIN): @$heading_title[$key];			
...
<?php if(isset($tmpdata['templatic_view_counter']) && $tmpdata['templatic_view_counter']=='Yes')
{
	...
	echo " ".$post_visit_count." ";
		($post_visit_count == 1)?_e('time',TDOMAIN):_e('times',TDOMAIN);
	echo ', '.$post_visit_daily_count." ";
		($post_visit_daily_count == 1)?_e("Visit today",TDOMAIN):_e("Visits today",TDOMAIN);
	echo "</p>";
	echo '</div>';
